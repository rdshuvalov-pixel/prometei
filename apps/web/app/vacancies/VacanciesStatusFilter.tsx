...
export function VacanciesStatusFilter({ onlyScored }: { onlyScored: boolean }) {
  const scoredHref = `/vacancies?filter=${encodeURIComponent(VACANCY_STATUS_SCORED)}`;
  return (
    <div className="mb-6 flex flex-wrap items-center gap-3">
      ...
      <Link className={onlyScored ? active : inactive} href={scoredHref}>
        Status = {VACANCY_STATUS_SCORED}
      </Link>
    </div>
  );
}
